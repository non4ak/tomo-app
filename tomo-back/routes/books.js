...
const { stripTags } = require('../utils/stringUtils');
    ...
    try {
        const response = await fetch(
            `https://www.googleapis.com/books/v1/volumes?q=subject:${encodeURIComponent(
                subject
            )}&country=${"UA"}&maxResults=8&printType=books&key=${API_KEY}`
        );
...
router.get("/:id", async (req, res) => {
    ...
    if (!bookId) {
        return res.status(400).json({ error: "Failed to fetch book. Missing id" });
    }
